...
async def handle_message(req: MessageRequest):
    ...
    try:
        ...
        context = {
            "tasks": [
                ...
                for r in conn.execute(
                    "SELECT id, title, priority, status, due_date FROM tasks WHERE status = 'bekliyor'"
                ).fetchall()
            ],
            "habits": [
                dict(r)
                for r in conn.execute(
                    "SELECT id, name, target FROM habits WHERE active = 1"
                ).fetchall()
            ],
            "events": [
                dict(r)
                for r in conn.execute(
                    "SELECT title, event_time FROM events WHERE DATE(event_time) >= ?", (today,)
                ).fetchall()
            ],
            "goals": [
                dict(r)
                for r in conn.execute(
                    "SELECT title, target_value, current_value, unit FROM goals WHERE status = 'aktif'"
                ).fetchall()
            ],
        }
        history = [
        ...
        conn.execute(
            "INSERT INTO messages (role, content) VALUES ('assistant', ?)", (final_response,)
        )
        conn.commit()
