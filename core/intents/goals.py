...
def handle_add_goal(conn: sqlite3.Connection, data: dict) -> str:
    ...
    conn.execute(
        "INSERT INTO goals (title, target_value, unit, deadline) VALUES (?, ?, ?, ?)",
        (title, target_value, unit, deadline),
    )
    conn.commit()
...
def handle_update_goal(conn: sqlite3.Connection, data: dict) -> str:
    ...
    goal = conn.execute(
        "SELECT id, title, target_value, current_value, unit FROM goals WHERE title LIKE ? AND status = 'aktif'",
        (f"%{title}%",),
    ).fetchone()
    if not goal:
...
def handle_goal_status(conn: sqlite3.Connection, data: dict) -> str:
    goals = conn.execute(
        "SELECT title, target_value, current_value, unit, deadline, status FROM goals WHERE status = 'aktif' ORDER BY deadline ASC"
    ).fetchall()
    if not goals:
    ...
    for g in goals:
        ...
        lines.append(
            f"• **{g['title']}**: {g['current_value']}/{g['target_value']} {g['unit']} [{bar}] %{pct}{deadline_str}"
        )
    return "\n".join(lines)
