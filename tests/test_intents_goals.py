...
def test_add_goal(db):
    result = handle_add_goal(
        db, {"title": "Kitap oku", "target_value": 10, "unit": "kitap", "deadline": "2026-04-30"}
    )
...
def test_update_goal(db):
    db.execute(
        "INSERT INTO goals (title, target_value, current_value, unit, status) VALUES ('Kitap oku', 10, 3, 'kitap', 'aktif')"
    )
    db.commit()
...
def test_update_goal_completes(db):
    db.execute(
        "INSERT INTO goals (title, target_value, current_value, unit, status) VALUES ('Kitap oku', 10, 9, 'kitap', 'aktif')"
    )
    db.commit()
...
def test_goal_status(db):
    db.execute(
        "INSERT INTO goals (title, target_value, current_value, unit, deadline, status) VALUES ('Kitap oku', 10, 6, 'kitap', '2026-04-30', 'aktif')"
    )
    db.commit()
