...
def test_add_task(db):
    result = handle_add_task(
        db, {"title": "Market alışverişi", "priority": "normal", "due_date": "2026-04-03"}
    )
...
def test_list_tasks_with_data(db):
    db.execute(
        "INSERT INTO tasks (title, priority, status) VALUES ('Görev 1', 'yüksek', 'bekliyor')"
    )
    db.execute(
        "INSERT INTO tasks (title, priority, status) VALUES ('Görev 2', 'normal', 'bekliyor')"
    )
    db.commit()
    result = handle_list_tasks(db, {})
