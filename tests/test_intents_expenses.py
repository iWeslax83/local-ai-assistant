...
def test_add_expense(db):
    result = handle_add_expense(
        db, {"amount": 450, "category": "market", "description": "Haftalık alışveriş"}
    )
...
def test_expense_summary(db):
    db.execute(
        "INSERT INTO expenses (amount, category, description) VALUES (450, 'market', 'Alışveriş')"
    )
    db.execute(
        "INSERT INTO expenses (amount, category, description) VALUES (50, 'ulaşım', 'Otobüs')"
    )
    db.execute(
        "INSERT INTO expenses (amount, category, description) VALUES (200, 'market', 'Manav')"
    )
    db.commit()
...
def test_expense_summary_weekly(db):
    db.execute(
        "INSERT INTO expenses (amount, category, description) VALUES (100, 'market', 'Test')"
    )
    db.commit()
