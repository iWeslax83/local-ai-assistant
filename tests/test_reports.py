...
def test_morning_summary_with_tasks(db):
    db.execute(
        "INSERT INTO tasks (title, priority, status, due_date) VALUES ('Test görevi', 'yüksek', 'bekliyor', DATE('now'))"
    )
    db.commit()
    result = build_morning_summary(db)
...
def test_morning_summary_with_events(db):
    db.execute(
        "INSERT INTO events (title, event_time) VALUES ('Toplantı', DATETIME('now', '+2 hours'))"
    )
    db.commit()
    result = build_morning_summary(db)
...
def test_monthly_report(db):
    ...
    db.execute(
        "INSERT INTO goals (title, target_value, current_value, unit, status) VALUES ('Test', 10, 5, 'adet', 'aktif')"
    )
    db.commit()
